...
def get_argparser():
    ...
    parser.add_argument("--conloss_sparsity", action="store_true")
    parser.add_argument("--KDLoss_prelogit", action="store_true")
    parser.add_argument("--conloss_compression", action="store_true")
...
def validate(opts, model, loader, device, metrics, alpha=None):
    ...
    all_class = True   
    if all_class:
        selected_cls = ['aeroplane', 'bicycle','bird','boat','bottle','bus','car','cat','chair','cow','diningtable','dog','horse','motorbike','person','pottedplant','sheep','sofa','train','tvmonitor']
    else:
    ...
    trainId2name = {0: 'background',1: 'aeroplane',2: 'bicycle', 3: 'bird',4: 'boat',5:'bottle',6: 'bus',7:'car',8:'cat',9:'chair',10:'cow',11:'diningtable',12:'dog',13:'horse',14:'motorbike',15:'person',16:'pottedplant',17:'sheep',18:'sofa',19:'train',20:'tvmonitor'}
    ...
    print(f'color {voc_cmap()[1].astype(np.uint8)}')
    trainId2color = {   0: voc_cmap()[0].astype(np.uint8),
                        1: voc_cmap()[1].astype(np.uint8),
                        2: voc_cmap()[2].astype(np.uint8),
                        3: voc_cmap()[3].astype(np.uint8),
                        4: voc_cmap()[4].astype(np.uint8),
                        5: voc_cmap()[5].astype(np.uint8),
                        6: voc_cmap()[6].astype(np.uint8),
                        7: voc_cmap()[7].astype(np.uint8),
                        8: voc_cmap()[8].astype(np.uint8),
                        9: voc_cmap()[9].astype(np.uint8),
                        10: voc_cmap()[10].astype(np.uint8),
                        11: voc_cmap()[11].astype(np.uint8),
                        12: voc_cmap()[12].astype(np.uint8),
                        13: voc_cmap()[13].astype(np.uint8),
                        14: voc_cmap()[14].astype(np.uint8),
                        15: voc_cmap()[15].astype(np.uint8),
                        16: voc_cmap()[16].astype(np.uint8),
                        17: voc_cmap()[17].astype(np.uint8),
                        18: voc_cmap()[18].astype(np.uint8),
                        19: voc_cmap()[19].astype(np.uint8),
                        20: voc_cmap()[20].astype(np.uint8)
                        }
    save_all_dirpath = ''
    # print(targets.shape)
    if not os.path.exists(save_all_dirpath):
        os.mkdir(save_all_dirpath)

    task_save_dir =  os.path.join(save_all_dirpath,str(opts.task))
    if not os.path.exists(task_save_dir):
        os.mkdir(task_save_dir)
    save_dir = os.path.join(task_save_dir,str(opts.curr_step))
    
    
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    save_dir_tsne = os.path.join(save_dir,'TSNE')
    output_dir = save_dir_tsne
    tsnecuda = False
    extention = '.png'
    duplication = 10
    plot_memory = False
    ...
    with torch.no_grad():
        for i, (images, labels, sal_maps, images_name) in enumerate(loader):
            ...
            # print(f'outputs_pixel {outputs_pixel.shape}')


            
            #feature_visual_fengzhi = feature
            #b,c,h,w = feature_visual_fengzhi.shape[0],feature_visual_fengzhi.shape[1],feature_visual_fengzhi.shape[2],feature_visual_fengzhi.shape[3]
            #feature_visual_fengzhi = F.interpolate(feature_visual_fengzhi, [513, 513], mode='bilinear', align_corners=True)
            
            #feature_visual_bj = outputs_pixel
            #b,c,h,w = feature_visual_bj.shape[0],feature_visual_bj.shape[1],feature_visual_bj.shape[2],feature_visual_bj.shape[3]
            #feature_visual_bj = F.interpolate(feature_visual_bj, [513, 513], mode='bilinear', align_corners=True)

           
            #feature_visual_rh = outputs_pixel
            #b,c,h,w = feature_visual_rh.shape[0],feature_visual_rh.shape[1],feature_visual_rh.shape[2],feature_visual_rh.shape[3]
            #feature_visual_rh = F.interpolate(feature_visual_rh, [513, 513], mode='bilinear', align_corners=True)

            #t-sne 
            #feature_tsne = outputs_pixel
            #B, C, H, W = images.shape
            #gt_image = labels.view(-1, H, W)[0].unsqueeze(0) 
            #print(f'gt_image {gt_image.shape}')
            
            

            #tsne_runner.input2basket(feature_tsne, gt_image, 'voc')
            ...
            #orin_image_dir = os.path.join(task_save_dir, 'orin_images')
            # if opts.curr_step == 0:
            #     if not os.path.exists(orin_image_dir):
            #         os.mkdir(orin_image_dir)
            # save_dir_gt = os.path.join(save_dir,'GT')
            # save_dir_self = os.path.join(save_dir,'Pred')
            # save_dir_feature = os.path.join(save_dir,'Feature_fengzhi')
            # save_dir_feature1 = os.path.join(save_dir,'Feature_boundary')
            # save_dir_feature2 = os.path.join(save_dir,'Feature_merge_attention')
            
            # if not os.path.exists(save_dir_gt):
            #     os.mkdir(save_dir_gt)
            # if not os.path.exists(save_dir_self):
            #     os.mkdir(save_dir_self)
            # if not os.path.exists(save_dir_feature):
            #      os.mkdir(save_dir_feature)
            #      os.mkdir(save_dir_feature1)
            #      os.mkdir(save_dir_feature2)
                

            # feature_visual_fengzhi_cpu = feature_visual_fengzhi.cpu().numpy()
            # feature_visual_bj_cpu = feature_visual_bj.cpu().numpy()
            # feature_visual_rh_cpu = feature_visual_rh.cpu().numpy()
            # # save_dir_feature = '/mnt/data/zhangxinyue/NAS/visual_featute_test'
            # # if not os.path.exists(save_dir_feature):
            # #     os.mkdir(save_dir_feature)
            # for batch in range(b):
            #     single_feature_fengzhi = feature_visual_fengzhi_cpu[batch]
                
            #     merged_feature_map_fengzhi = np.mean(single_feature_fengzhi, axis=0)
            #     merged_feature_map_fengzhi = 1.0 / (1 + np.exp(-1 * merged_feature_map_fengzhi))
            #     # to [0,255]
            #     merged_feature_map_fengzhi = np.round(merged_feature_map_fengzhi * 255)
            #     #plt.imshow(merged_feature_map_fengzhi, cmap='viridis')  
            #     #plt.axis('off')  
            #     #plt.savefig(os.path.join(save_dir_feature,str(i)+'_' +str(batch)+ '.png')) 
            #     #plt.close()
            #     plt.imsave(os.path.join(save_dir_feature,images_name[batch]+'.png'), merged_feature_map_fengzhi, cmap='viridis')

               
            #     single_feature_bj = feature_visual_bj_cpu[batch]
            #     merged_feature_map_bj = np.max(single_feature_bj, axis=0)
            #     #merged_feature_map = 1.0 / (1 + np.exp(-1 * merged_feature_map))
            #     # to [0,255]
            #     # merged_feature_map = np.round(merged_feature_map * 255)
            #     plt.imsave(os.path.join(save_dir_feature1,images_name[batch]+'.png'), merged_feature_map_bj, cmap='viridis')
                
            #     single_feature_rh = feature_visual_rh_cpu[batch]
            #     merged_feature_map_rh = np.sum(single_feature_rh, axis=0)
            #     #  merged_feature_map
            #     min_val = np.min(merged_feature_map_rh)
            #     max_val = np.max(merged_feature_map_rh)
            #     normalized_feature_map = (merged_feature_map_rh - min_val) / (max_val - min_val)

            #     img_color = cv2.applyColorMap((normalized_feature_map * 255).astype(np.uint8), cv2.COLORMAP_JET)  

            #     mean = [0.485, 0.456, 0.406]
            #     std = [0.229, 0.224, 0.225]
            #     denormalized_image = normalize(images[batch], [-mean[i] / std[i] for i in range(3)], [1 / std[i] for i in range(3)])

            #     image = denormalized_image.cpu().numpy()
            ...
            #     merged_feature_map = 0.5*rgb_image + 0.5*img_color
            #     cv2.imwrite(os.path.join(save_dir_feature2,images_name[batch]+'.png'),merged_feature_map)

            # for index in range(preds.shape[0]):
               
            #     pred = voc_cmap()[preds[index].astype(int)].astype(np.uint8)
            #     if images_name[index][:-4]=='.jpg' or images_name[index][:-4]=='.png':
            #         #plt.imsave(os.path.join(save_dir_self,images_name[index]), pred)
            #         plt.imsave(os.path.join(save_dir_self,images_name[batch]+'.png'), pred)
            #         target_color = voc_cmap()[targets[index].astype(int)].astype(np.uint8)
            #         #plt.imsave(os.path.join(save_dir_gt,images_name[index]), target_color)
            #         plt.imsave(os.path.join(save_dir_gt,images_name[batch]+'.png'), target_color)
            
            #         # Define the mean and standard deviation for normalization
            #         mean = [0.485, 0.456, 0.406]
            #         std = [0.229, 0.224, 0.225]
            #         denormalized_image = normalize(images[index], [-mean[i] / std[i] for i in range(3)], [1 / std[i] for i in range(3)])
            ...
            #         # pil_image = Image.fromarray(image)
            #         rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            #         #cv2.imwrite(os.path.join(orin_image_dir,images_name[index]),rgb_image)
            #         cv2.imwrite(os.path.join(orin_image_dir,images_name[batch]+'.png'),rgb_image)
            #     else:
            #         plt.imsave(os.path.join(save_dir_self,images_name[batch]+'.png'), pred)
            #         target_color = voc_cmap()[targets[index].astype(int)].astype(np.uint8)
            #         plt.imsave(os.path.join(save_dir_gt,images_name[batch]+'.png'), target_color)
            #         # print(f' images[index]{ images[index].shape}')
...
def main(opts):
    ...
    print(f'opt.dataset{opts.dataset}')
    print('lalallal')
    print(opts.task)
    ...
    if opts.freeze and opts.curr_step > 0:
        ...
        for param in model.classifier.parameters():  # classifier for new class
            param.requires_grad = True
        #Encouraging all parameters to be updated as new knowledge is learned
        lr_index = 1.0 / np.exp(opts.curr_step) * 1e-3
        lr_index1 = 1.0 / np.exp(opts.curr_step) * 1e-2
        training_params = [{'params': model.backbone.parameters(), 'lr': opts.lr * lr_index},
                          ...
                          {'params': model.classifier.head[1:-1].parameters(), 'lr': opts.lr * lr_index1},
                          {'params': model.classifier.head[-1].parameters(), 'lr': opts.lr},
                          {'params': model.classifier.head2[0].parameters(), 'lr': opts.lr},
                          {'params': model.classifier.head2[1:-1].parameters(), 'lr': opts.lr * lr_index1},
                          {'params': model.classifier.head2[-1].parameters(), 'lr': opts.lr},
                          {'params': model.classifier.proposal_head[0].parameters(), 'lr': opts.lr},
                          {'params': model.classifier.proposal_head[1:-1].parameters(), 'lr': opts.lr * lr_index1},
    ...
    if opts.curr_step > 0:  # previous step checkpoint
        #if os.path.exists(ckpt_str % (opts.name, opts.model, opts.dataset, opts.task, opts.curr_step)):
        #    print('exist current step log,continue training')
        #    opts.ckpt = ckpt_str % (opts.name, opts.model, opts.dataset, opts.task, opts.curr_step)
        #else:
    ...
    # ==========   Train Loop   ==========#
    if opts.test_only:
        print(f'test ckpt_dir {ckpt_str}')
    ...
    while cur_itrs < total_itrs:
        ...
        with torch.cuda.amp.autocast(enabled=opts.amp):
            ...
            if (opts.conloss_sparsity or opts.conloss_compression) and opts.curr_step > 0:
                if opts.conloss_sparsity:
